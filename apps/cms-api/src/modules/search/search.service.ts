...
//         const idx = this.normalize(text).indexOf(this.normalize(q));
...
//                     excerpt: this.highlight(title, q),
//                 });
//             }
//         }
...
private decodeHtmlEntities(text: string): string {
    ...
    /**
     * Escapes HTML and wraps every occurrence of `q` (diacritic-insensitive)
     * in <strong> tags. Relies on normalize() being a 1-to-1 char mapping,
     * so indices in normalized text line up with the original text.
     */
